...
\author{学号：\_\_\_\_\_\_\_\_\_\_\quad 姓名：\_\_\_\_\_\_\_\_\_\_}
\date{\today}
...
\subsubsection{批大小}
由于CPU训练时间限制，本组数据采用模型3个epoch的快速训练结果对比趋势：

从实验结果可知，批大小64的验证准确率最高（98.38\%），
批大小16收敛最快但存在一定波动，批大小256收敛最稳定但最终准确率略低。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{cm_exp1_CNN_Better.pdf}
\caption{最佳模型CNN\_Better的混淆矩阵}
\label{fig:cm_best}
\end{figure}
...
\begin{tabular}{lcccc}
\toprule
\textbf{影响因素} & \textbf{最佳设置} & \textbf{最高准确率} & \textbf{影响程度} \\
\midrule
网络结构 & CNN\_Better & 99.36\% & 最大（$\Delta=2.14\%$） \\
损失函数 & CrossEntropy & 98.39\% & 较小（$\Delta=1.12\%$） \\
学习率 & $10^{-3}$ & 99.00\% & 中等（$\Delta=1.20\%$） \\
正则化 & 无/Dropout小 & 98.39\% & 显著（过强Dropout=-3.39\%） \\
\bottomrule
\end{tabular}
\end{table}
